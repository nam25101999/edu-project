...
public class GraduationConditionController {
    ...
    public ResponseEntity<GraduationConditionResponseDTO> create(@Valid @RequestBody GraduationConditionRequestDTO requestDTO) {
        return new ResponseEntity<>(graduationConditionService.create(requestDTO), HttpStatus.CREATED);
    }

    @GetMapping
    public ResponseEntity<List<GraduationConditionResponseDTO>> getAll() {
        return ResponseEntity.ok(graduationConditionService.getAll());
    }
